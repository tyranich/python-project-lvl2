        ...
        def test_(dict1, level_, string_):
            ...
            for _ in childs:
                if isinstance(childs[_], dict):    
                    ...
                    string_.append((tabs * (level_)) + "{}: {}".format(_, childs[_]) + "\n")
            level_ -= 1
    ...
    def create_str(return_str, level, _dict, key=None):
        ...
        for _ in _dict.keys():
            if _dict[_]["status"] == "dict":
                return_str.append( "{}{}: {{\n".format(tabs_two * (level + DOUBLE_IDENT), _))
                create_str(return_str, level + 2, get_value(_dict[_]))
                return_str.append("{}}}\n".format(tabs_two * (level + DOUBLE_IDENT)))
            elif _dict[_]["status"] == "added":
                if isinstance(get_value(_dict[_]), dict):
                    return_str.append( "{}+ {}: {{\n".format(tabs_two * (level + SING_IDENT), _,))
                    с = "".join(dict_string(get_value(_dict[_]), level + DOUBLE_IDENT))
                    return_str.append("{}".format(с))
                else:    
                    return_str.append("{}+ {}: {}\n".format(tabs_two * (level + SING_IDENT), _, get_value(_dict[_])))
                ...
                if isinstance(_dict[_]["value1"], dict):
                    return_str.append( "{}- {}: {{\n".format(tabs_two * (level + SING_IDENT), _,))
                    с = "".join(dict_string(_dict[_]["value1"], level + DOUBLE_IDENT))
                    return_str.append("{}".format(с))
                else:
                    return_str.append( "{}- {}: {}\n".format(tabs_two * (level + SING_IDENT), _, _dict[_]["value1"]))

                if  isinstance(_dict[_]["value2"], dict):
                    return_str.append( "{}+ {}: {{\n".format(tabs_two * (level + SING_IDENT), _,))
                    с = "".join(dict_string(_dict[_]["value2"], level + DOUBLE_IDENT))
                    return_str.append("{}".format(с))
                else:
                    return_str.append( "{}+ {}: {}\n".format(tabs_two * (level + SING_IDENT), _, _dict[_]["value2"]))
            elif _dict[_]["status"] == "not changed":
                return_str.append( "{}{}: {}\n".format(tabs_two * (level + DOUBLE_IDENT), _, get_value(_dict[_])))
